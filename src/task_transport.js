...
const fillExtension = {
    ...
    target: (creep) => {
        ...
        if (creep.fillExtensions()) return ERR_NOT_FOUND
        return false
    }
}
...
const fillTower = {
    ...
    target: (creep) => {
        if (creep.isEmpty) return true
        if (creep.fillTowers()) return ERR_NOT_FOUND
        return false
    }
}
...
const labEnergy = {
    ...
    target: (creep) => {
        if (creep.isEmpty) return true
        if (creep.fillLabEnergy()) return ERR_NOT_FOUND
        return false
    }
}
...
const labBoostIn = {
    ...
    source: (creep) => {
        ...
        const result = creep.getFrom(creep.room.resAvailable(lab.boostType), lab.boostType)
...
const labContainerOut = {
    ...
    source: (creep) => {
        if (creep.isFull) return true
        if (!creep.room.labContainer) return ERR_NOT_FOUND
...
const labReactionIn = {
    ...
    source: (creep) => {
        ...
        if (!sourceType) return ERR_NOT_FOUND
        const result = creep.getFrom(creep.room.resAvailable(sourceType), sourceType)
        if (result === OK) return true
        else if (result === ERR_NOT_IN_RANGE) return false
        return result
    },
    target: (creep) => {
        ...
        if (!sourceType || !lab) return ERR_NOT_FOUND
