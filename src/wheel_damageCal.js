...
let pro = {
    ...
    possibleTowerDamage (room, pos) {
        return _.sum(room.tower, tower => {
            ...
            return damageCal.calTowerDamage(tower.pos.getRangeTo(pos)) * ratio;
        });
    },
    ...
    possibleCreepDamage (body, dis, risk = false) {
        return damageCal.possibleCreepNearDamage(body, dis, risk) + damageCal.possibleCreepRangeDamage(body, dis, risk);
    },
    ...
    /**
     * 计算一个位置可能接受的治疗值
     * @param {RoomPosition} pos 要计算治疗值的位置
     * @param {Creep[]} creeps 位置周围的治疗兵
     */
    possibleHealHits (pos, creeps) {
        return _.sum(creeps, c => damageCal.getHeal(c.body, pos.getRangeTo(c)));
    },
    ...
    possibleDamage (body, pos, username, heal = true, towerDamage = 0, risk = false) {
        ...
        let possibleDamage = _.sum(attackers, attacker => damageCal.possibleCreepDamage(attacker.body, pos.getRangeTo(attacker), risk)) + (towerDamage || 0);
        possibleDamage = damageCal.hitsOnTough(body, possibleDamage);
        let possibleHeal = 0;
        if (heal) {
            let healers = pos.findInRange(FIND_CREEPS, 3, {filter: creep => creep.owner.username === username && creep.bodyCounts[HEAL]});
            possibleHeal = damageCal.possibleHealHits(pos, healers);
        }
    ...
    wouldBreakDefend (body, pos, username, towerDamage = 0, risk = false) {
        return damageCal.possibleDamage(body, pos, username, true, towerDamage, risk) > 0;
    }
}

global.damageCal = pro
